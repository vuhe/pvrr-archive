...
use selectors::{matching, parser, Element, OpaqueElement};
use std::fmt;
use std::sync::Arc;
...
impl CssSelector {
    ...
    pub(crate) fn matches(&self, scope: NodeRef<'_, Node>, element: NodeRef<'_, Node>) -> bool {
        let scope = CssElement(scope);
        let element = CssElement(element);
...
impl<'a> Element for CssElement<'a> {
    ...
    fn parent_element(&self) -> Option<Self> {
        self.0.parent().filter(|it| it.value().is_element()).map(|it| Self(it))
    }
    ...
    fn is_same_type(&self, other: &Self) -> bool {
        self.0.value().as_html_tag().map(|it| &it.name)
            == other.0.value().as_html_tag().map(|it| &it.name)
    }
    ...
    fn prev_sibling_element(&self) -> Option<Self> {
        self.0.prev_siblings().find(|sibling| sibling.value().is_element()).map(|it| Self(it))
    }

    fn next_sibling_element(&self) -> Option<Self> {
        self.0.next_siblings().find(|sibling| sibling.value().is_element()).map(|it| Self(it))
    }

    fn is_html_element_in_html_document(&self) -> bool {
        // FIXME: Is there more to this?
        self.0.value().as_html_tag().map_or(false, |it| it.name.ns == ns!(html))
    }

    fn has_local_name(&self, name: &CssLocalName) -> bool {
        self.0.value().as_html_tag().map_or(false, |it| it.name.local == name.0)
    }

    fn has_namespace(&self, namespace: &Namespace) -> bool {
        self.0.value().as_html_tag().map_or(false, |it| it.name.ns == *namespace)
    }
    ...
    fn is_link(&self) -> bool {
        self.0.value().as_html_tag().map_or(false, |it| &*it.name.local == "link")
    }
    ...
    fn has_id(&self, id: &CssLocalName, case_sensitivity: CaseSensitivity) -> bool {
        match self.0.value().as_html_tag().and_then(|it| it.id()) {
    ...
    fn is_empty(&self) -> bool {
        !self.0.children().any(|child| child.value().is_element() || child.value().is_value())
    }

    fn is_root(&self) -> bool {
        self.0.parent().map_or(false, |parent| parent.value().is_root())
    }
}
